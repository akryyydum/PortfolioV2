...
import {
  ...
  IconTable, IconVolume, IconVolume2, IconVolume3,
...
export const MacbookScroll = ({
  ...
  /** Live JSX rendered onto the MacBook screen instead of a static image */
  ...
  // Lid opens during first 30% of scroll, then stays put — no translateY drift
  const translate = useTransform(scrollYProgress, [0, 0.3], [0, 0]);
  ...
  return (
    <div
      ref={ref}
      className="relative flex min-h-[160vh] shrink-0 scale-[0.35] transform flex-col items-center justify-start py-0 [perspective:800px] sm:scale-50 md:scale-100"
    >
      {/* Sticky wrapper keeps the MacBook pinned in the center of the viewport
          while the parent div's extra height provides the scroll travel */}
      <div className="sticky top-0 flex min-h-screen w-full flex-col items-center justify-center py-20">
      <motion.h2
      ...
      <Lid src={src} screenContent={screenContent} scaleX={scaleX} scaleY={scaleY} rotate={rotate} translate={translate} />
...
export const Lid = ({
  scaleX, scaleY, rotate, translate, src, screenContent,
}: {
...
export const KBtn = ({ className, children, childrenClassName, backlit = true }: { className?: string; children?: React.ReactNode; childrenClassName?: string; backlit?: boolean }) => (
